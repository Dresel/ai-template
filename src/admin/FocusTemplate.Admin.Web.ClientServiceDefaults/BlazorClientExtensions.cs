...
public static class BlazorClientExtensions
{
	...
	private static WebAssemblyHostBuilder ConfigureBlazorClientOpenTelemetry(this WebAssemblyHostBuilder builder, string? serviceName)
	{
		...
		Uri otlpEndpoint = new(baseAddress, $"{otlpPathBase}/");
		...
		{
			// TODO: Enable logging for the OTLP exporter once the circular dependency is fixed in .NET 11 preview 6.
		...
		builder.Services.AddOpenTelemetry()
			...
			.WithTracing(tracing =>
			{
				tracing.AddSource("Microsoft.AspNetCore.Components").AddHttpClientInstrumentation();
